	...
	namespace Circulators 
	{
		...
		*/		
		...
		public:
			...
			/** construct with mesh and a half edge */
			VertexHedronIterT(mesh_ref _mesh, const typename HedronContainer::iterator & _start, const typename HedronContainer::iterator & _h_end, bool _end = false) : 
			    mesh_(&_mesh),
			...
			/** construct with mesh and a half face */
			VertexHedronIterT(mesh_ref _mesh, HedronContainer _hedrons, bool _end = false) : 
			    mesh_(&_mesh),
				hedrons_(_hedrons),
				start_(0),
				hh_(0),
		...
		*/		
		...
		public:
			...
			/** construct with mesh and a half edge */
			HedronHalfEdgeIterT(mesh_ref _mesh, const HalfEdgeHandle & _start, unsigned int _he_count, bool _end = false) : 
			                    mesh_(&_mesh),
								start_(_start),
								heh_(_start),
								he_count_(_he_count),
		...
		public:
			...
			*/
			operator bool() const
			{
				return ((heh_ >=0) && ((heh_.idx() - start_.idx()) < he_count_) && ((heh_.idx() - start_.idx()) > -1 )) && ((start_ != heh_) || (lap_counter_ == 0));
			}
		private:
			mesh_ptr       mesh_;
			HalfEdgeHandle start_;
		...
		*/		
		...
		public:
			...
			/** construct with mesh and a half edge */
			ConstHedronHalfEdgeIterT(mesh_ref _mesh, const HalfEdgeHandle & _start, unsigned int _he_count, bool _end = false) : 
			    mesh_(&_mesh),
				start_(_start),
				heh_(_start),
				he_count_(_he_count),

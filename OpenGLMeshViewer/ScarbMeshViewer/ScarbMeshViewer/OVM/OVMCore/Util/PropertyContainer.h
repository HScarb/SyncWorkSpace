...
//---------------------------------------------------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------------------------------------------
	protected:
		unsigned int _add(BaseProperty * _bp)
		{
			iterator p_end(properties_.end());
			unsigned int idx;
			idx = 0;
		...
		BaseProperty & _property(unsigned int _idx)
		{
		...
		const BaseProperty & _property(unsigned int _idx) const
		{
			assert(_idx < properties_.size());
			assert(properties_[_idx] != NULL);
			return *properties_[_idx];
	...
	protected:
		...
		struct Swap
		{
			Swap(unsigned int _i0, unsigned int _i1) : i0_(_i0), i1_(_i1) 
			{
